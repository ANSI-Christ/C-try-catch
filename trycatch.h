...
#define _THROW1(_type_,...) ({\
    struct _TRYCATCH * const _1_=_TryCatch();\
    if(_1_ && _1_->jmp){\
        M_WHEN(M_IS_ARG(M_PEAK(__VA_ARGS__)))( const struct{int pad1[32];_type_ data;int pad2[32];}_2_={{0},__VA_ARGS__}; _1_->e->data.c=&_2_.data; )\
        _1_->e->type=M_STRING(_type_); _1_->e->where=_THROW_INFO; longjmp(*_1_->jmp,1);\
